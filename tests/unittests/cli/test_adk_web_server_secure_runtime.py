...
def test_run_live_uses_secure_runtime_wrapped_runner(
    tmp_path: Path, monkeypatch
) -> None:
  app_root = tmp_path / 'courtroom'
  app_root.mkdir()
  ...
  async def _run_live(self, *args, **kwargs):
    del args, kwargs
    plugin_names[:] = [plugin.name for plugin in self.app.plugins]
    yield Event(author='runner')

  monkeypatch.setattr(Runner, 'run_live', _run_live)
  ...
  with client.websocket_connect(
      '/run_live?app_name=courtroom&user_id=user&session_id=session'
  ) as ws:
    _ = ws.receive_text()
